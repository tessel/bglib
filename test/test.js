...
// Packet Creation:
var response = bglib.getPacket(bg.api.systemHello);
assert(response.cID == 1 
	&& response.cClass == 0, "Invalid packet created.");
console.log("Simple Packet Creation Passed.")

response = bglib.getPacket(bg.api.gapConnectDirect, [new Buffer([108, 189, 40, 93, 28, 216]), 1, 25, 50, 500, 8]);
assert(response.cID == 3 
	&& response.cClass == 6, "Invalid packet created.");
console.log("Complex Packet Creation Passed.");

assert.throws(function() {
	bglib.getPacket(bg.api.systemHello, ["Invalid", "params"])
	console.log("Invalid parameters FAILED.");
});
console.log("Detecting Invalid Parameters Passed.");

assert.throws(function() {
	bglib.getPacket(bg.api.gapConnectDirect, [108, 189, 40, 93, 28, 216], 1, 25, 50, 500, 8);
	console.log("Invalid parameters FAILED.");
});
